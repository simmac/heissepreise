...
exports.generateCategories = async function () {
    ...
    for (const baseSlug of baseCategorySlugs) {
        const data = await axios.get(`https://shop.billa.at/api/product-discovery/categories/${baseSlug}/child-properties`);
...
exports.fetchData = async function () {
    ...
    for (const category of categories) {
        let page = 0;
        while (true) {
            const data = await axios.get(`https://shop.billa.at/api/product-discovery/categories/${category.id}/products?pageSize=500&page=${page}`);
            page++;
